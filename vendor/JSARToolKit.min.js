 ...
 *
 * ????
 * Java?Generics?????????????Vector.&lt;*&gt;????????????Vector.&lt;Object&gt;???
 * ?????????????????????????????????????
 * strict = false ??????????????
 * ???????????????????????
 */
...
NyARBufferType = Klass(
    (function() {
        ...
        return ({
            ...
            /**
             * int[][]?0/1?2???
             * ???????1bit?BUFFERFORMAT_INT2D_GRAY_1??????
             */
...
FLARRasterFilter_Threshold = ASKlass('FLARRasterFilter_Threshold',
        ...
        ,setThreshold : function(i_threshold )
    {
        ...
        ,doFilterOtsuThreshold : function(i_input, i_output)
    {
        ...
        //Calculate intensity (greyscale) from RGB data
        for (var i=0,j=0; i<d.length; i+=4,++j) {
            var c = d[i]*2989+d[i+1]*5866+d[i+2]*1145;
            obd[j] = Math.round(c/10000);
        }

        //Set-up histogram
        $('histData', 0);
        // constructors
        this.histData = new Array(256);
        ...
        //Calculate histogram
        ptr = 0;
        while(ptr < obd.length){
            var h =obd[ptr];
            this.histData[h] ++ ;
            ptr ++ ;
        }

        //CALCULATE OTSU THRESHOLD (http://www.labbookpages.co.uk/software/imgProc/otsuThreshold.html)
        var total = obd.length;
        var sum = 0;
        for(
            var t = 0;t < 256; t ++ )
        ...
        //console.log("Otsu threshold: "+ th3);

        //Assing black or white pixel according to the determined threshold
        for (var i=0; i<obd.length; i++) {
            obd[i] = (obd[i] <= th3) ? 0xffffffff : 0xff000000;
        }

        //Display binary image on debug screen
        if (window.DEBUG) {
    ...
    ,doFilterFixedThreshold : function(i_input, i_output)
{
    NyAS3Utils.assert (i_input._width == i_output._width && i_input._height == i_output._height);
    var out_buf = (i_output.getBuffer());
    var in_reader = i_input.getRgbPixelReader();
...
NyARLabeling_Rle = ASKlass('NyARLabeling_Rle',
        ...
        ,addFragment : function(i_rel_img,i_img_idx,i_nof,i_row_index,o_stack)
    {
        ...
        var len=r - l;
        i_rel_img[i_img_idx+fidx] = i_nof;// REL????ID
        ...
        for (var y = i_top + 1; y < i_bottom; y++) {
            ...
            SCAN_CUR: for (i = 0; i < len_current; i++) {
                ...
                SCAN_PREV: while (index_prev < len_prev) {
                    if (rle_current[i*off+lidx] - rle_prev[index_prev*off+ridx] > 0) {// 0??8???????
                        // prev?cur??????????????????
                        index_prev++;
                        continue;
                    } else if (rle_prev[index_prev*off+lidx] - rle_current[i*off+ridx] > 0) {// 0??8??????????
                        // prev?cur??????????????
...
NyARParam = ASKlass('NyARParam',
        ...
        ,getDistortionFactor : function()
    {
         ...
         *
         * @param i_factor
         * NyARCameraDistortionFactor??????????????????4??????
         * @param i_projection
         * NyARPerspectiveProjectionMatrix?????????????????12??????
         */
...
NyARMatchPattDeviationBlackWhiteData = ASKlass('NyARMatchPattDeviationBlackWhiteData',
        ...
        ,setRaster : function(i_raster)
    {
        ...
        // input???????wh???// input=new int[height][width][3];
...
NyARTransMat = ASKlass('NyARTransMat',INyARTransMat,
    {
        ...
        NyARTransMat : function(i_param)
        {
            ...
            //???NyARRotMatrix_NyARToolKit?NyARRotMatrix_ARToolKit?????????????????
...
NyARPartialDifferentiationOptimize = ASKlass('NyARPartialDifferentiationOptimize',
    {
         ...
         * cosa)*iy)+cp12*(i_trans.z+(sina)*iy)); double h =((-cosa*ix)*sinb +(cosa*iz)*cosb +i_trans.z+(sina)*iy); ???????? L=2*S(d[n]*e[n]+a[n]*b[n])
         * J=2*S(d[n]*f[n]+a[n]*c[n])/L K=2*S(-e[n]*f[n]+b[n]*c[n])/L M=S(-e[n]^2+d[n]^2-b[n]^2+a[n]^2)/L ???? +J*cos(x) +K*sin(x) -sin(x)^2 +cos(x)^2
         * +2*M*cos(x)*sin(x)
         */
...
NyARRotMatrix = ASKlass('NyARRotMatrix',NyARDoubleMatrix33,
    {
        ...
        __initRot_vec2 : null,
        /**
         * NyARTransMatResult?????NyARRotMatrix???????
         * @param i_prev_result
         */
...
/**
 * NyARColorPatt_NyIdMarker??????Perspective??????????????
 *
 */
PerspectivePixelReader = ASKlass('PerspectivePixelReader',
        ...
        ,checkFreqWidth : function(i_freq,i_width)
    {
        ...
        /**
         * i_freq_count_table?i_freq_table????????????????????????
        ...
        ,getMaxFreq : function(i_freq_count_table,i_freq_table,o_freq_table)
    {
        ...
        //???????????????(this.FRQ_POINTS*this.FRQ_STEP?100????????????)
        FRQ_EDGE : 10,
        FRQ_STEP : 2,
        ...
        //(model+1)*4*3?this.THRESHOLD_PIXEL*3?????????
...
NyARCustomSingleDetectMarker = ASKlass('NyARCustomSingleDetectMarker',
        ...
        ,getConfidence : function()
    {
         ...
         *
         * @param i_is_continue
         * TRUE???transMatCont?????????? FALSE???transMat??????????
         */
...
NyARDetectMarker = ASKlass('NyARDetectMarker',
    {
         ...
         * @param i_code
         * ?????????ARCode?????????
         * ???????????????????getARCodeIndex???????ARCode????????????
         * ??????[1]?ARCode????????????????getARCodeIndex?1??????
         * @param i_marker_width
         * i_code????????????????????????????? ????i_number_of_code????????????????????????
         * @param i_number_of_code
         * i_code??????ARCode?????????
         * @param i_input_raster_type
         * ?????????????????????????INyARBufferReader????????getBufferType???????????
         * @throws NyARException
         */
        ...
        i_input_raster_type)
    {
        ...
        /**
         * i_index??????????????????????o_result??????? ???????detectMarkerLite???????????????
         *
         * @param i_index
         * ???????????????????? ???????detectMarkerLite????????0?????????????
         * @param o_result
        ...
        ,getARCodeIndex : function(i_index)
    {
         ...
         *
         * @param i_is_continue
         * TRUE???transMatContinue??????? FALSE???transMat???????
         */
        ,setContinueMode : function(i_is_continue)
    {
        this._is_continue = i_is_continue;
...
NyARSingleDetectMarker = ASKlass('NyARSingleDetectMarker', NyARCustomSingleDetectMarker,
    {
        ...
        PF_TEST2 : 201,
        /**
         * ????ARCode????????????1??ARCode?????NyARSingleDetectMarker????????????
         *
         * @param i_param
         * ???????????????
         * @param i_code
...
//      trace('w:'+markerWidthByDec+'/h:'+markerHeightByDec);
        ...
        ,setContinueMode : function(i_is_continue)
    {
         ...
         *  ?????????????30px~230px????????
         *  640x480?????????i_max??????????????
         *  ???0 ???????? FLARLabeling.AR_AREA_MAX?FLARLabeling.AR_AREA_MIN????????
         *
...
FLARMultiIdMarkerDetector = ASKlass('FLARMultiIdMarkerDetector',
        ...
        ,detectMarkerLite : function(i_raster, i_threshold)
    {
        ...
        // ????2??????????.
        //NEW IMPLEMTATION: automatic threshold with OTSU
        //this._tobin_filter.setThreshold(i_threshold);
        //this._tobin_filter.doFilterFixedThreshold(i_raster, this._bin_raster);
        this._tobin_filter.doFilterOtsuThreshold(i_raster, this._bin_raster);
        // ??????????(???????????????????????????)
...
TransformedBitmapPickup = ASKlass('TransformedBitmapPickup', NyARColorPatt_Perspective_O2,
    {
         ...
         * ??????????????
         * @param i_resolution
         * resolution of reading pixel per point. ---- ????????????????1??????????????????2?????????
         */
         ...
         * ----
         * ??????basement????????A?????????????????????????
         * ????8cm????RECT(i_l,i_t,i_r,i_b)?-40,0,0,-40.0?????????????????????????
         *
         * ?????????????????????????????????????????????
         * @param i_src_imege
         * ???????????????
